...
void RobustSqrtAndInvSqrt(const Matrix &Mat, Matrix &MatSqrt,
  ...
  Vector EigVal = Vector((SAES.eigenvalues().array() > Tolerance)
                             .select(SAES.eigenvalues().array(), 0));
  Vector EigValInv =
      Vector((SAES.eigenvalues().array() > Tolerance)
                 .select(SAES.eigenvalues().array().inverse(), 0));

  /** use modified eigen values to compute sqrt */
  MatSqrt = SAES.eigenvectors() * EigVal.cwiseSqrt().asDiagonal() *
            SAES.eigenvectors().transpose();
  MatSqrtInv = SAES.eigenvectors() * EigValInv.cwiseSqrt().asDiagonal() *
               SAES.eigenvectors().transpose();
}
