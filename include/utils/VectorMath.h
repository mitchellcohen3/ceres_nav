...
template <int Dim, typename T> MatrixT<T, Dim, Dim> SquareRoot(MatrixT<T, Dim, Dim> A) {
  /** compute SVD */
  Eigen::SelfAdjointEigenSolver<MatrixT<T, Dim, Dim>> SAES(A);

  /** compute tolerance (idea from OKVIS) */
  T Tolerance =
      std::numeric_limits<T>::epsilon() * A.cols() * SAES.eigenvalues().array().maxCoeff();

  /** set small eigen values to zero */
  VectorT<T, Dim> EigVal = VectorT<T, Dim>(
      (SAES.eigenvalues().array() > Tolerance).select(SAES.eigenvalues().array(), 0));
...
template <int Dim, typename T> MatrixT<T, Dim, Dim> InverseSquareRoot(MatrixT<T, Dim, Dim> A) {
  /** compute SVD */
  Eigen::SelfAdjointEigenSolver<MatrixT<T, Dim, Dim>> SAES(A);
  ...
  /** set small eigen values to zero */
  VectorT<T, Dim> EigValInv = Vector(
      (SAES.eigenvalues().array() > Tolerance).select(SAES.eigenvalues().array().inverse(), 0));
...
template <int Dim, typename T> MatrixT<T, Dim, Dim> Inverse(MatrixT<T, Dim, Dim> A) {
  /** compute SVD */
  Eigen::SelfAdjointEigenSolver<MatrixT<T, Dim, Dim>> SAES(A);
...
template <int Dim, typename T> T VectorLength(const T *const Vector) {
...
template <int Dim> bool IsPositiveSemidefinite(MatrixStatic<Dim, Dim> Mat) {
